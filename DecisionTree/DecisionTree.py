...
CAR_TRAINING = "./datasets/car/train.csv"
CAR_TESTING = "./datasets/car/test.csv"
BANK_TRAINING = "./datasets/bank/train.csv"
BANK_TESTING = "./datasets/bank/test.csv"

# Car Problem Setup
with open(CAR_TRAINING, mode='r') as f:
    train_car=[];
    for line in f:
        terms=line.strip().split(',') # 7*N matrix
        train_car.append(terms)
...
with open(CAR_TESTING, mode='r') as f:
    test_car = [];
    for line in f:
        terms = line.strip().split(',')  # 7*N matrix
        test_car.append(terms)
...
def create_list_car(attribute):
    obj = {}
    for attr_val in car_attributes[attribute]:
        obj[attr_val] = []
    return obj
...
def data_split_car(attributes, dataset):
    branch_obj = create_list_car(attributes)
    for row in dataset:
        for attr_val in car_attributes[attributes]:
            if row[car_pos(attributes)] == attr_val:
...
def tree_build_car(train, max_depth):
    root = find_best_split(train, "car")
    child_node(root, max_depth, 1, "car")
    return root
...
def label_predict_car(node, inst):
    if isinstance(node[inst[car_pos(node['best_attr'])]], dict):
        return label_predict_car(node[inst[car_pos(node['best_attr'])]], inst)
    else:
        return node[inst[car_pos(node['best_attr'])]]  # leaf node
...
def calc_prediction_error_training_car(tree):
    true_label = []
    pred_seq = []
    for row in train_car:
        true_label.append(row[-1])
        pre = label_predict_car(tree, row)
        pred_seq.append(pre)
    return error(true_label, pred_seq)


def calc_prediction_error_testing_car(tree):
    true_label = []
    pred_seq = []
    for row in test_car:
        true_label.append(row[-1])
...
# Error output for car experiment
tree_depth_1 = tree_build_car(train_car, 1)
tree_depth_2 = tree_build_car(train_car, 2)
tree_depth_3 = tree_build_car(train_car, 3)
tree_depth_4 = tree_build_car(train_car, 4)
tree_depth_5 = tree_build_car(train_car, 5)
tree_depth_6 = tree_build_car(train_car, 6)
print("******Prediction Error in Training Data Set for Car******")
print(calc_prediction_error_training_car(tree_depth_1))
print(calc_prediction_error_training_car(tree_depth_2))
print(calc_prediction_error_training_car(tree_depth_3))
print(calc_prediction_error_training_car(tree_depth_4))
print(calc_prediction_error_training_car(tree_depth_5))
print(calc_prediction_error_training_car(tree_depth_6))
print("******Prediction Error in Testing Data Set for Car******")
print(calc_prediction_error_testing_car(tree_depth_1))
print(calc_prediction_error_testing_car(tree_depth_2))
print(calc_prediction_error_testing_car(tree_depth_3))
print(calc_prediction_error_testing_car(tree_depth_4))
print(calc_prediction_error_testing_car(tree_depth_5))
print(calc_prediction_error_testing_car(tree_depth_6))

# Error output for bank experiment
